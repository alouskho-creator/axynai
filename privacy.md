syllas – Privacy Policy

Last updated: February 2026

This Privacy Policy describes how syllas (“we”, “our”, or “us”) collects, uses, and protects information when you use our products and services, including syllas and any future applications developed by syllas.

By using our services, you agree to the collection and use of information in accordance with this policy.

⸻

1. Information We Collect

a. Account Information

When you create an account, we may collect:
	•	Email address
	•	Name or username
	•	Authentication data (such as Google Sign-In)

Authentication is handled securely using third-party providers such as Firebase Authentication.

b. User Content

When using our AI-powered features, we may process:
	•	Chat messages
	•	Prompts
	•	Uploaded text or content you voluntarily provide

This content is used solely to provide and improve the service.

c. Usage Data

We may collect limited technical data such as:
	•	Device type
	•	App version
	•	Interaction data
This helps us improve performance and reliability.

⸻

2. How We Use Information

We use collected information to:
	•	Provide and operate our services
	•	Authenticate users
	•	Improve app functionality and user experience
	•	Maintain security and prevent abuse
	•	Comply with legal obligations

We do not sell personal data.

⸻

3. AI Processing

User-provided content may be processed by AI systems to generate responses or results.
AI outputs are generated automatically and may not always be accurate.

Users are responsible for how they use AI-generated content.

⸻

4. Data Storage and Security

We use industry-standard security practices and trusted infrastructure providers (such as Firebase) to protect user data.

However, no method of electronic storage is 100% secure, and we cannot guarantee absolute security.

⸻

5. Data Retention

We retain personal data only as long as necessary to provide our services or comply with legal requirements.

Users may request deletion of their data at any time.

⸻

6. User Rights

You have the right to:
	•	Access your personal data
	•	Request correction or deletion of your data
	•	Delete your account through app settings (where available)

⸻

7. Third-Party Services

Our services may rely on third-party tools such as:
	•	Authentication providers
	•	Cloud infrastructure
	•	Analytics services

These providers are only given access to information necessary to perform their services.

⸻

8. Children’s Privacy

Our services are not intended for children under the age of 13.
We do not knowingly collect personal information from children.

⸻

9. Changes to This Policy

We may update this Privacy Policy from time to time.
Changes will be posted on this page with an updated revision date.

⸻

10. Contact Us

If you have any questions about this Privacy Policy or your data, you may contact us at:

syllas
Email: support@syllas.com

⸻

© 2026 syllas. All rights reserved.
